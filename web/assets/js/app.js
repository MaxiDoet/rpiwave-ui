...
function setCurrentlyPlaying(playing, mediaTitle, sourceType, applicationNum, pageNum) {
...
function playWebStation(data) {
    ...
    setCurrentlyPlaying(true, data["name"], 0, 0, 0)
}
...
function displayWeather() {
	...
	request.onload = function() {
		...
		try{
			...
			console.log("Received invalid weatherdata!");
		}

		// We need to implement all weathercodes here because otherwise sometimes it'd just display nothing!
		var weatherIcons = {
			rain_heavy: "fa-cloud-showers-heavy",
			rain: "fa-cloud-showers",
			rain_light: "fa-cloud-showers",
			freezing_rain_heavy: "fa-cloud-sleet",
			freezing_rain: "fa-cloud-sleet",
			freezing_rain_light: "fa-cloud-sleet",
			freezing_drizzle: "fa-cloud-sleet",
			drizzle: "fa-cloud-drizzle",
			ice_pellets_heavy: "fa-cloud-hail-mixed",
			ice_pellets: "fa-cloud-hail",
			ice_pellets_light: "fa-cloud-hail",
			snow_heavy: "fa-cloud-snow",
			snow: "fa-cloud-snow",
			snow_light: "fa-cloud-snow",
			flurries: "fa-cloud-snow",
			tstorm: "fa-thunderstorm",
			fog_light: "fa-fog",
			fog: "fa-fog",
			cloudy: "fa-clouds",
			mostly_cloudy: "fa-cloud",
			partly_cloudy: "fa-clouds-sun",
			mostly_clear: "fa-cloud-sun",
			clear: "fa-sun"
}
		// First we'll have to remove the old icon
		for (var i =0; i < weatherWidgetIcon.classList.length;i++) {
 			if(weatherWidgetIcon.classList[i].startsWith("fa-")) {
				weatherWidgetIcon.classList.remove(weatherWidgetIcon.classList[i]);
    			}
		}
...
document.querySelector('#page-4-home').addEventListener("click", function() {
    stopPlayback()
    setCurrentlyPlaying(false, "", 0, 0, 0)
    setPage(2)
})
...
setInterval(function() {
	displayWeather();
}, 300000); // Update weatherwidget every 5min
